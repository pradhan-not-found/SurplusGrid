...
export default function ProducerDashboard() {
  ...
  const exportCSV = () => {
    const header = "Date,Start Time,End Time,Energy (kWh),Status\n";
    const rows = availableSurplus.map(s => `${s.date},${s.startTime},${s.endTime},${s.energyKwh},${s.status}`).join("\n");
    ...
    link.href = url;
    link.setAttribute('download', `predicted_windows_${new Date().toISOString().split('T')[0]}.csv`);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 glass-card p-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={exportCSV} className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors">
                    Export CSV
                  </button>
                </div>
